...
\end{empheq}-->
where $P(x|y)$ is the probability that $x$ is true given $y$ is true. Rearranging this gives
<!--\begin{empheq}[box={\borderedmathbox}]{equation*}
    P(B|A,I) = \frac{P(A|B,I)P(B|I)}{P(A|I)}.
\end{empheq}-->
This is <!--\href{https://en.wikipedia.org/wiki/Bayes\%27\_theorem}{{\color{violet}{\bf Bayes' theorem}}}-->.

Note: we will use $P(A,B|I)$ to denote the probability of $A$ _and_ $B$, where we have also 
explicitly added the conditioning on background information $I$.
...
the model (we'll explore this briefly later, but at the moment note it as a constant normalisation factor 
for the posterior)
...
You may only be interested in the distribution of one (or some subset) of the parameters, so you <!--\href{https://en.wikipedia.org/wiki/Marginal\_distribution}{\bf \color{violet}{marginalise}}--> (i.e., integrate) over the <!--\href{https://en.wikipedia.org/wiki/Nuisance\_parameter}{\bf \color{violet}{nuisance parameters}}-->. E.g., if we're only interested in the gradient of the line then:
<!--\begin{empheq}[box={\borderedmathbox[scale=0.9]}]{equation*}
...
<!--{\footnotesize If you are purely interested in marginal posterior distributions the normalisation is not important and can most often
...
\begin{empheq}[box={\borderedmathbox[scale=0.9]}]{equation*}
\mathcal{O}_{12} \equiv \redub{\frac{p(\mathcal{H}_1|\bm{d}, I)}{p(\mathcal{H}_2|\bm{d}, I)}}_{\mathclap{\text{Model Odds}}} = \redob{p(\frac{\bm{d}|\mathcal{H}_1, I)}{p(\bm{d}|\mathcal{H}_2, I)}}^{\mathclap{\text{Bayes Factor}}} \redob{\frac{p(\mathcal{H}_1| I)}{p(\mathcal{H}_2| I)}}^{\mathclap{\text{Prior Odds}}}.
\end{empheq}
{\footnotesize The \href{https://en.wikipedia.org/wiki/Bayes\_factor}{\bf \color{violet}{Bayes factor}} is the ratio of the evidences for the two hypotheses. The prior odds defines the {\it a priori} relative degree-of-belief about each model, which in practice is often set to unity, i.e., neither model is preferred.}-->
...
that <!--\href{https://en.wikipedia.org/wiki/Maximum\_entropy\_probability\_distribution\#Specified\_variance:\_the\_normal\_distribution}{\bf \color{violet}{maximises the entropy}}-->, i.e., is the least informative (most conservative), is the normal distribution:
<!--\begin{empheq}[box={\borderedmathbox[scale=0.9]}]{equation*}
...
In this case, with a fixed value of $\sigma$, this is a <!--\href{https://en.wikipedia.org/wiki/Stationary\_process}{strictly (or strongly) \color{violet}{\bf stationary process}}-->.
...
<!--{\scriptsize As an example, we'll examine the problem of fitting a line $\bm{y} = m\bm{x}+c$ to data, $\bm{d}$ \href{https://en.wikipedia.org/wiki/Linear_regression}{\color{violet}{\bf linear regression}}). We can write the posterior for
the parameters}
...
\end{empheq}
{\scriptsize If the prior on the parameters is uniform and independent, so}
\begin{empheq}[box={\borderedmathbox[scale=0.8]}]{equation*}
p(m,c|I) = p(m|I)p(c|I) = \text{constant},
\end{empheq}
{\scriptsize then the posterior is}
\begin{empheq}[box={\borderedmathbox[scale=0.8]}]{equation*}
p(m,c|\bm{d},I) \propto p(\bm{d}|m,c,I).
\end{empheq}
{\scriptsize We could, in this case, use the machinery of \href{https://en.wikipedia.org/wiki/Maximum\_likelihood\_estimation}{\bf \color{violet}{maximum likelihood estimation}}, e.g., \href{https://en.wikipedia.org/wiki/Ordinary\_least\_squares}{\bf \color{violet}{least squares fitting}}, to estimate the parameters.}-->
...
\end{empheq}-->
where $\bm{d}$ is the data, $\bm{\theta}$ is a set of parameters defining a waveform model $s$, the tilde
represents the Fourier transform, and $S_n(f)$ is the one-sided power spectral density of the noise in the data.
...
p(\bm{d}|\bm{\theta}, I) \propto \exp{\left(-\frac{1}{2} \left[ 4\Re \textcolor{red}{\sum_{i=j}^k} \frac{\left(\tilde{d}_i - \tilde{s}_i(\bm{\theta})\right)\left(\tilde{d}_i - \tilde{s}_i(\bm{\theta})\right)^*}{\textcolor{red}{T} S_n(f_i)} \right]\right)},
\end{empheq}-->
due to $\int \dots {\rm d}f \approx \sum \dots \Delta f$, and $\Delta f = 1/T$ for data of length $T$ seconds,
and $i$ is the index over frequency bins from bin $j$ to $k$.
...
we get
<!--\begin{empheq}[box={\borderedmathbox[scale=0.75, left=10mm, right=10mm]}]{align*}
p(\bm{d}|\bm{\theta}, I)  & \propto \exp{\left(-\frac{1}{2} \left[ 4 \sum_{i=j}^k \frac{\textcolor{red}{\Re}\left(\tilde{d}_i - \tilde{s}_i(\bm{\theta})\right)^2 + \textcolor{red}{\Im}\left(\tilde{d}_i - \tilde{s}_i(\bm{\theta})\right)^2}{T S_n(f_i)} \right]\right)}, \nonumber \\
& \equiv \exp{\left(-\frac{1}{2} \left[ 4 \sum_{i=j}^k \frac{\textcolor{red}{\left|\tilde{d}_i - \tilde{s}_i(\bm{\theta})\right|^2}}{T S_n(f_i)} \right]\right)}
\end{empheq}-->
...
<!--\begin{empheq}[box={\borderedmathbox[scale=0.8]}]{equation*}
\ln{p(\bm{d}|\bm{\theta}, I)} = -\frac{1}{2} \left[ 4 \sum_{i=j}^k \frac{\Re\left(\tilde{d}_i - \tilde{s}_i(\bm{\theta})\right)^2 + \Im\left(\tilde{d}_i - \tilde{s}_i(\bm{\theta})\right)^2}{T S_n(f_i)} \right] + C,
\end{empheq}-->
where $C$ is the normalisation term given by <!--\citep[see Equation 12 of][]{2015PhRvD..91d2003V}-->
<!--\begin{empheq}[box={\borderedmathbox[scale=0.8]}]{equation*}
C = -\frac{1}{2}\sum_{i=j}^k \ln{\left(\pi T S_n(f_i) / 2 \right)}.
\end{empheq}-->
...
\end{empheq}-->
Evaluating the likelihood over the parameter space essentially requires evaluating these $\rho^2$ terms.
...
\end{empheq}-->
Windowing is _vital_ (when Fourier tranforming the analysis segment and for the PSD estimation) to prevent
